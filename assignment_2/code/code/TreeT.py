import random
from DecisionNodeT import DecisionNodeT
import graphviz
...
class TreeT:
    ...
    feature_names =  ["AGE", "HbA1c", "Chol", "TG", "BMI"]
    ...
    def generate_random_individual(self, max_depth=4, method='grow', population=20):
        ...
        def create_grow_individual(depth):
            if depth >= max_depth or (depth >= 3 and random.random() < 0.4):
                ...
                return DecisionNodeT(value=value)
                ...
                if feature == 'AGE':
                    min_val, max_val = 20, 60
                elif feature == 'HbA1c':
                    min_val, max_val = 1, 13
                elif feature == 'Chol':
                    min_val, max_val = 1, 10
                elif feature == 'TG':
                    min_val, max_val = 0.5, 8
                elif feature == 'BMI':
                    min_val, max_val = 19, 47
                ...
                return DecisionNodeT(feature=feature, threshold=threshold, left=left, right=right)
    ...
    def perform_crossover(self, parent1, parent2):
        ...
        offspring1 = TreeT(self.feature_names)
        offspring1.root = parent1_copy
        offspring2 = TreeT(self.feature_names)
    ...
    def prune_tree_max_height(self, node, max_height):
        ...
        if self.get_tree_height(node.left) > max_height - 1:
            ...
            node.left = None
            node.left = DecisionNodeT(value=self.get_majority_class(node))
        ...
        if self.get_tree_height(node.right) > max_height - 1:
            ...
            node.right = None
            node.right = DecisionNodeT(value=self.get_majority_class(node))
