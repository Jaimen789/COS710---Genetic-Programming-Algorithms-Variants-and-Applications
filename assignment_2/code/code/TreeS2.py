import random
from DecisionNodeS2 import DecisionNodeS2
import graphviz
...
class TreeS2:
    ...
    def generate_random_individual(self, max_depth=4, method='grow', population=20):
        ...
        def create_grow_individual(depth):
            if depth >= max_depth or (depth >= 3 and random.random() < 0.4):
                ...
                return DecisionNodeS2(value=value)
                ...
                if feature == 'bmi':
                    ...
                    min_val, max_val = 80, 300
                elif feature == 'HbA1c_level':
                    min_val, max_val = 2, 9
                elif feature == 'age':
                    min_val, max_val = 4, 80
                ...
                return DecisionNodeS2(feature=feature, threshold=threshold, left=left, right=right)
    ...
    def perform_crossover(self, parent1, parent2):
        ...
        offspring1 = TreeS2(self.feature_names)
        offspring1.root = parent1_copy
        offspring2 = TreeS2(self.feature_names)
    ...
    def prune_tree_max_height(self, node, max_height):
        ...
        if self.get_tree_height(node.left) > max_height - 1:
            ...
            node.left = None
            node.left = DecisionNodeS2(value=self.get_majority_class(node))
        ...
        if self.get_tree_height(node.right) > max_height - 1:
            ...
            node.right = None
            node.right = DecisionNodeS2(value=self.get_majority_class(node))
