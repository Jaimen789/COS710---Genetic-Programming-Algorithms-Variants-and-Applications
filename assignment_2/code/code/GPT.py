import os
import random
from TreeT import TreeT



# random.seed(42)

class GeneticProgrammingT:
    
    feature_names =  ["AGE", "HbA1c", "Chol", "TG", "BMI"]
    
    def __init__(self, population_size=100, seed=55):
        self.population = [TreeT(self.feature_names) for _ in range(population_size)]
        self.seed = seed
        random.seed(seed)
    ...
    def evolve_population(self, X_train, y_train):
        ...
        # print("Head of X_train_target:")
        # print(X_train.head())

        # print("\nHead of y_train_target:")
        # print(y_train.head())
    ...
    def make_prediction(self, node, sample):
        ...
        # Check if the feature is present in the sample
        if node.feature in sample:
    ...
    def evaluate_fitness(self, predictions, actual_labels):
        ...
        for pred, actual in zip(predictions, actual_labels['CLASS']):
            if pred == actual:
