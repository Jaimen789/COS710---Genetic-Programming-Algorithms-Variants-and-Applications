...
class GeneticProgramming:
    ...
    def __init__(self, population_size=100, seed=55):
        self.population = [Tree(self.feature_names) for _ in range(population_size)]
        self.seed = seed
        random.seed(seed)
    ...
    def evolve_population(self, X_train, y_train):
        ...
        parents = self.perform_tournament_selection()
        parent1_individual, parent1_fitness = parents[0]
        parent2_individual, parent2_fitness = parents[1]
    ...
    def perform_tournament_selection(self):
        ...
        winner_indices = sorted(range(len(fitness_scores)), key=lambda i: fitness_scores[i], reverse=True)[:2]
        parent1_individual = tournament_candidates[winner_indices[0]]
        parent2_individual = tournament_candidates[winner_indices[1]]
        ...
        return (parent1_individual, fitness_scores[winner_indices[0]]), (parent2_individual, fitness_scores[winner_indices[1]])
    ...
    #     # Convert predictions and actual_labels to NumPy arrays for element-wise comparison
    ...
    def evaluate_fitness(self, predictions, actual_labels):
        ...
        for pred, actual in zip(predictions, actual_labels['Outcome']):
            if pred == actual:
